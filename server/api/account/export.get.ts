...
export default defineEventHandler(async (event) => {
  ...
  if (cvsError) throw createError({ statusCode: 500, message: cvsError.message })

  const { data: snapshots, error: snapshotsError } = await client.from('cv_snapshots').select('*')
  if (snapshotsError) throw createError({ statusCode: 500, message: snapshotsError.message })
